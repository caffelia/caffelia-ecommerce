<template v-if="isLoading">
    <x-caffelia-shop::shimmer.datagrid.toolbar />
</template>
    ...
    <div class="mt-7 flex items-center justify-between gap-4 max-md:block">
        ...
        <div class="flex w-full gap-x-1">
            ...
            <template v-if="applied.massActions.indices.length">
                <x-caffelia-shop::datagrid.toolbar.mass-action>
            ...
            <!-- Search Panel -->
            <template v-else>
                <x-caffelia-shop::datagrid.toolbar.search>
                    <template #search="{
                        available,
                        applied,
                        search,
                ...
                <div class="hidden w-11 max-md:block">
                    <x-caffelia-shop::datagrid.toolbar.filter>
                        <template #filter="{
                            available,
                            applied,
                            filters,
        ...
        <div class="flex gap-x-4 max-md:my-4 max-md:items-center max-md:justify-between">
            <!-- Pagination Panel -->
            <x-caffelia-shop::datagrid.toolbar.pagination>
                <template #pagination="{
                    available,
                    applied,
                    changePage,
            ...
            <div class="hidden max-md:block ltr:pl-2.5 rtl:pr-2.5">
                <p class="text-sm font-light text-gray-800 max-md:w-full">
                    @{{ "@lang('caffelia-shop::app.components.datagrid.toolbar.results')".replace(':total', available.meta.total) }}
                </p>
            </div>
