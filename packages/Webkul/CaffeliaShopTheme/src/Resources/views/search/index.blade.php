@php
    $title = request()->has('query')
            ? trans('caffelia-shop::app.search.title', ['query' => request()->query('query')])
            : trans('caffelia-shop::app.search.results');
@endphp
...
<x-caffelia-shop::layouts :has-feature="false">
    <!-- Page Title -->
    <x-slot:title>
        {{ $title }}
    </x-slot>
    ...
    <div class="container px-[60px] max-lg:px-8 max-sm:px-4">
        @if (request()->has('image-search'))
            @include('caffelia-shop::search.images.results')
        @endif
    ...
    <!-- Product Listing -->
    <v-search>
        <x-caffelia-shop::shimmer.categories.view />
    </v-search>
        ...
        >
            <div class="container px-[60px] max-lg:px-8 max-sm:px-4">
                <div class="flex items-start gap-10 max-lg:gap-5 md:mt-10">
                    ...
                    @include('caffelia-shop::categories.filters')
                    ...
                    <div class="flex-1">
                        ...
                        >
                            ...
                            <template v-if="isLoading">
                                <x-caffelia-shop::shimmer.products.cards.list count="12" />
                            </template>

                            <!-- Product Card Listing -->
                            <template v-else>
                                <template v-if="products.length">
                                    <x-caffelia-shop::products.card
                                        ::mode="'list'"
                                ...
                                <template v-else>
                                    <div class="m-auto grid w-full place-content-center items-center justify-items-center py-32 text-center">
                                        ...
                                        <p
                                            ...
                                            role="heading"
                                        >
                                            @lang('caffelia-shop::app.categories.view.empty')
                                        </p>
                                    </div>
                                </template>
                            </template>
                        ...
                        <div v-else>
                            ...
                            <template v-if="isLoading">
                                <div class="mt-8 grid grid-cols-3 gap-8 max-1060:grid-cols-2 max-md:gap-x-4 max-sm:mt-5 max-sm:justify-items-center max-sm:gap-y-5">
                                    <x-caffelia-shop::shimmer.products.cards.grid count="12" />
                                </div>
                            </template>
                        ...
                        <button
                            ...
                            v-if="links.next"
                        >
                            @lang('caffelia-shop::app.categories.view.load-more')
                        </button>
                    </div>
                </div>
            </div>
