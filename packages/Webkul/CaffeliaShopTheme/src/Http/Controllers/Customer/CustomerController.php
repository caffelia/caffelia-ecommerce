...
namespace Caffelia\ShopTheme\Http\Controllers\Customer;
...
use Caffelia\ShopTheme\Http\Controllers\Controller;
use Caffelia\ShopTheme\Http\Requests\Customer\ProfileRequest;
...
class CustomerController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('caffelia-shop::customers.account.profile.index', compact('customer'));
    }
    ...
    public function edit()
    {
        ...
        return view('caffelia-shop::customers.account.profile.edit', compact('customer'));
    }
    ...
    public function update(ProfileRequest $profileRequest)
    {
        ...
        if (! empty($data['current_password'])) {
            if (Hash::check($data['current_password'], auth()->guard('customer')->user()->password)) {
                ...
                session()->flash('warning', trans('caffelia-shop::app.customers.account.profile.index.unmatched'));
        ...
        if ($customer = $this->customerRepository->update($data, auth()->guard('customer')->user()->id)) {
            ...
            session()->flash('success', trans('caffelia-shop::app.customers.account.profile.index.edit-success'));

            return redirect()->route('shop.customers.account.profile.index');
        }

        session()->flash('success', trans('caffelia-shop::app.customer.account.profile.edit-fail'));
        ...
        try {
            if (Hash::check(request()->input('password'), $customerRepository->password)) {
                if ($customerRepository->orders->whereIn('status', [Order::STATUS_PENDING, Order::STATUS_PROCESSING])->first()) {
                    session()->flash('error', trans('caffelia-shop::app.customers.account.profile.index.order-pending'));
                ...
                session()->flash('success', trans('caffelia-shop::app.customers.account.profile.index.delete-success'));

                return redirect()->route('shop.customer.session.index');
            }

            session()->flash('error', trans('caffelia-shop::app.customers.account.profile.index.wrong-password'));

            return redirect()->back();
        } catch (\Exception $e) {
            session()->flash('error', trans('caffelia-shop::app.customers.account.profile.index.delete-failed'));
    ...
    public function reviews()
    {
        ...
        return view('caffelia-shop::customers.account.reviews.index', compact('reviews'));
    }
    ...
    {
        return view('caffelia-shop::customers.account.index');
    }
}
